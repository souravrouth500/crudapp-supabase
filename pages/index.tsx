...
function Home() {
  ...
  const [studentList, setStudentList] = React.useState<StudentsResponse[]>([])
  const queryClient = useQueryClient()
  const { data: studentsListResponse }: ResponseObj = useQuery({
  ...
  React.useEffect(() => {
    if(searchInput.length > 1){
      const filtered = studentList?.data?.filter(item => {
        // console.log(item);
